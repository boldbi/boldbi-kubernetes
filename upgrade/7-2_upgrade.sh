...
then
	...
	then
		...
		if [ ! -f "boldbi_7-2/service.yaml" ]; then curl -o boldbi_7-2/service.yaml https://raw.githubusercontent.com/boldbi/boldbi-kubernetes/v7.2.7/deploy/service.yaml; fi
		if [ ! -f "boldbi_7-2/hpa.yaml" ]; then curl -o boldbi_7-2/hpa.yaml https://raw.githubusercontent.com/boldbi/boldbi-kubernetes/v7.2.7/deploy/hpa.yaml; fi
		if [ ! -f "boldbi_7-2/deployment.yaml" ]; then curl -o boldbi_7-2/deployment.yaml https://raw.githubusercontent.com/boldbi/boldbi-kubernetes/v7.2.7/deploy/deployment.yaml; fi
